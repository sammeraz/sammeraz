  ...
  if (items.length === 0) {
    return (
      <div className="border border-ink/10 bg-white px-8 py-12 text-center">
        <h3 className="font-display text-2xl text-ink">Your cart is empty</h3>
  ...
  return (
    <div className="grid gap-16 md:grid-cols-[1.1fr_0.9fr] md:gap-24">
      ...
      <div className="flex flex-col gap-6">
        ...
        <ul className="flex flex-col gap-5 border-t border-ink/15 pt-6">
          {items.map((item) => (
            <li key={item.slug} className="flex items-start justify-between gap-4">
              <div>
                <h3 className="font-display text-lg leading-none text-ink">{item.title}</h3>
                <p className="mt-1 text-xs uppercase tracking-[0.08em] text-ink/50">
        ...
        <p className="text-xs text-ink/45">Shipping and any applicable tax are quoted by email.</p>
      </div>
    </div>
  );
}
